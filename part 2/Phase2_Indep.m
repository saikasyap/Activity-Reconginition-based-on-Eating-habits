...
output1 = strcat(folderPath,'\Assignment 2\Phase 2');
mkdir(output1);
...
for k = 19 : size(userNums, 1)
   ...
   X_Test = [UsersEat_Newmat;UsersNotEat_Newmat(1:size(UsersEat_Newmat, 1), :)];
   Y_Test = [ones(size(UsersEat_Newmat, 1), 1); zeros(size(UsersEat_Newmat, 1), 1)];
...
writematrix(User_data, fullfile(output1, "user_indep_data.csv"));
